...
logger.debug("Available secrets keys: %s", list(st.secrets.keys()))
...
try:
    ...
    logger.debug("Using ChromaDB at %s", chroma_path)
    ...
    try:
        ...
        logger.debug("Using existing collection")
    except:
        ...
        logger.debug("Created new collection")
except Exception as e:
...
def read_and_chunk_pdf(pdf_path):
    logger.debug("Reading and chunking PDF: %s", pdf_path)
